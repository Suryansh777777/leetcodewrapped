...
import { Trophy, Calendar, Star, Medal, Code, Search, Loader2 } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen p-8 bg-gradient-to-br from-zinc-900 via-zinc-800 to-zinc-900 text-white">
      <div className="max-w-6xl mx-auto">
        ...
        <p className="text-zinc-400 text-center mb-12">Visualize your LeetCode journey and achievements</p>
              ...
              <CardContent>
                <div className="space-y-2">
                  {data.profile?.languageProblemCount
                    ?.sort((a: LanguageStats, b: LanguageStats) => 
                      (b.problemsSolved || 0) - (a.problemsSolved || 0)
                    )
                    ?.slice(0, 5)
                    ?.map((lang: LanguageStats) => (
                      <div key={lang.languageName} className="flex justify-between items-center">
                        <span className="text-gray-400">{lang.languageName || 'Unknown'}</span>
                        <div className="flex items-center gap-2">
                          <Progress 
                            value={((lang.problemsSolved || 0) / (data.profile?.submitStats?.acSubmissionNum?.[0]?.count || 1)) * 100} 
                            className="w-20"
                          />
